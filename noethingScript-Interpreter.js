"use strict";
// 解释器版本
const NSIVersion = "2.0.0";
// console.log("NSI Version: " + NSIVersion);
// Debug级别变量
var DEBUG_LEVEL = 0; // 默认不输出debug信息
// 自定义debug日志函数
...
(function (ExceptionType) {
    ...
    ExceptionType["UNKNOWN_ERROR"] = "UnknownError";
...
class ScopeManager {
    ...
    // 添加变量 (全局或局部) 
    static addVariable(name, value, type, startLine, endLine, isGlobal = false, isConst = false) {
        ...
        if (isGlobal) {
            if (GLOBAL_VARS.hasOwnProperty(name)) {
                console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 引用错误: 名称 '${name}' 已被定义`);
    ...
    // 获取变量值 (考虑行号作用域) 
    static getVariable(vname, currentLine, isArray = false, isGlobal = false) {
        let name = vname;
    ...
    // 获取变量信息 (考虑行号作用域) 
    static getVariableInfo(vname, currentLine, isGlobal = false) {
        let name = vname;
    ...
    // 获取变量类型
    static getVariableType(vname, currentLine, isGlobal = false) {
        let name = vname;
        if (isGlobal) {
            if (name.startsWith('global.')) {
                name = name.slice('global.'.length);
    ...
    // 设置变量值
    static setVariable(vname, value, currentLine, isGlobal = false) {
        let name = vname;
        ...
        if (!isGlobal) {
            ...
            for (let i = LOCAL_VARS.length - 1; i >= 0; i--) {
                ...
                if (varInfo.name === name && currentLine >= varInfo.startLine && (currentLine <= varInfo.endLine || varInfo.endLine === -1)) {
                    // 检查是否为常量
                    if (varInfo.isConst) {
                        console.error(`TypeError: Cannot assign to constant variable '${name}' at line ${currentLine + 1}`);
                        return false;
                    }
                    // 验证类型
                    const validation = ScopeManager.validateType(value, varInfo.type);
                    if (!validation.isValid) {
                        console.error(`TypeError: Cannot assign value '${value}' to variable '${name}' of type '${varInfo.type}' at line ${currentLine + 1}`);
                        return false;
                    }
        ...
        console.error(`引用错误: 变量 '${name}' 在第 ${currentLine + 1} 行未定义`);
        return false;
    }
    // 检查变量是否存在
    static hasVariable(vname, currentLine, isGlobal = false) {
        let name = vname;
    ...
    // 注册函数
    static registerFunction(funcInfo) {
    ...
    // 获取当前行所在的函数名
    static getCurrentFunction(currentLine) {
    ...
    // 从函数信息中提取函数定义的返回值变量名
    static getReturnVarName(funcInfo) {
    ...
    // }
    // 清理指定全局变量 (手动管理)
    static cleanupGlobalVariable(varName) {
    ...
    static cleanupLocalVariable(cleanAll = false, exceptMode, varName, startLine, endLine) {
        if (cleanAll) {
            LOCAL_VARS = [];
        ...
        if (!cleanAll && !varName && startLine !== undefined && endLine !== undefined) {
            ...
            console.error("未指定清除所有变量且未指定要清除的变量");
            return;
        }
    }
    // 检查函数返回类型是否为undefined (void函数) 
    static isVoidFunction(funcInfo) {
...
class Interpreter {
    ...
    static scanTagsAndFunctions() {
        ...
        for (let i = 0; i < programLines.length; i++) {
            ...
            if (line.indexOf(':') === 0) {
                ...
                if (line === ':end') {
                    if (!inFunction) {
                        console.error(`函数结束标记错误: 第 ${i + 1} 行发现函数结束标记, 但没有对应的函数定义`);
                        return;
                    }
                    if (currentFunction) {
                        // 更新FUNCTION中的函数作用域信息
                        ...
                        if (currentFunction.returnType !== DataType.UNDEFINED) {
                            ...
                            if (!hasReturn) {
                                console.error(`错误: 函数 ${currentFunction.name} 期望返回 ${currentFunction.returnType} 类型的值, 但未找到return语句 直到第 ${i + 1} 行`);
                            }
                        }
                        else {
                            ...
                            if (hasReturn) {
                                console.error(`错误: 函数 ${currentFunction.name} 期望无返回值, 但找到return语句 在第 ${returnLine + 1} 行`);
                            }
                        }
                    }
                    inFunction = false;
                ...
                // 检查是否是其他函数相关标记
                if (inFunction) {
                    console.error(`函数定义错误: 第 ${i + 1} 行发现函数内嵌套函数定义`);
                    return;
                }
                ...
                if (funcMatch) {
                    ...
                    if (!Interpreter.isValidIdentifier(funcName)) {
                        console.error(`[ERROR 4] [Line ${i + 1}]命名错误: 函数名 '${funcName}' 不符合命名规则 (参考C语言规则)  在第 ${i + 1} 行`);
                        return;
                    }
                    ...
                    if (returnVarNameOrVoid === ':void') {
                        ...
                        if (!returnTypeStr) {
                            console.error(`函数返回值格式错误: 第 ${i + 1} 行, 有返回值的函数必须指定返回值类型`);
                            return;
                        }
        ...
        if (inFunction) {
            console.error(`函数定义错误: 程序结束时仍有未结束的函数`);
        }
        debugLog(1, `扫描标签和函数定义结束`);
    }
    // 辅助方法: 将字符串类型转换为DataType枚举
    static getDataTypeFromString(typeStr) {
        switch (typeStr.toLowerCase()) {
            ...
            default:
                console.error(`不支持的数据类型: ${typeStr}`);
                return DataType.UNDEFINED;
        }
    }
    // 辅助方法: 从值推断数据类型
    ...
    // 辅助方法: 解析值并进行类型检查
    static parseValue(valueStr, expectedType) {
        // 处理字符串 (必须带双引号) 
        if (valueStr.startsWith('"') && valueStr.endsWith('"')) {
            const strValue = valueStr.substring(1, valueStr.length - 1);
            if (expectedType !== DataType.STRING) {
                throw { type: ExceptionType.TYPE_ERROR, message: `类型不匹配: 期望 ${expectedType}, 实际是字符串` };
            }
            return strValue;
        }
        // 处理布尔值
        if (valueStr === 'true' || valueStr === 'false') {
            ...
            if (expectedType !== DataType.BOOL) {
                throw { type: ExceptionType.TYPE_ERROR, message: `类型不匹配: 期望 ${expectedType}, 实际是布尔值` };
            }
            return boolValue;
        }
        // 处理数字 (包括十六进制、二进制、八进制) 
        ...
        if (/^(-?0[xX][0-9a-fA-F]+|-?0[bB][01]+|-?0[oO][0-7]+|-?\d+\.\d+|-?\d+)$/.test(valueStr)) {
            ...
            if (expectedType === DataType.NUMBER) {
                ...
                if (!Number.isInteger(numValue)) {
                    throw { type: ExceptionType.TYPE_ERROR, message: `类型不匹配: 期望整数, 实际是 ${numValue}` };
                }
                ...
                throw { type: ExceptionType.TYPE_ERROR, message: `类型不匹配: 期望 ${expectedType}, 实际是数字` };
            }
        }
        // 处理变量引用
        ...
        if (varValue !== undefined) {
            ...
            if (varType !== expectedType) {
                throw { type: ExceptionType.TYPE_ERROR, message: `类型不匹配: 期望 ${expectedType}, 实际是 ${varType}` };
            }
            return varValue;
        }
        throw { type: ExceptionType.SYNTAX_ERROR, message: `无法解析值: ${valueStr}` };
    }
    // 执行程序
    static run() {
        currentLinePointer = 0;
        ...
        while (currentLinePointer < programLines.length) {
            try {
                ...
                if (tryIdx !== -1) {
                    const tryFrame = CONTROL_FLOW_STACK[tryIdx];
                ...
                console.error(`错误: ${exception.message} (行 ${currentLinePointer + 1})`);
                // 根据异常类型决定继续执行还是终止
    ...
    static executeCommand(command) {
        ...
        if (cmd === 'const' && parts.length > 1) {
            ...
            if (nextCmd === 'global' || nextCmd === 'local') {
                console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 全局/局部变量声明格式应为 "[global/local] [const] 变量名:类型 = 值"`);
                return;
            }
        }
        switch (cmd) {
            case 'global':
                ...
                if (parts.length > 1 && parts[1].toLowerCase() === 'const') {
                    ...
                    if (parts[1].toLowerCase().startsWith('const')) {
                        console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 全局变量声明格式应为 "global [const] 变量名:类型 = 值"`);
                        return;
                    }
                    Interpreter.executeGlobal(parts.slice(1).join(' '));
                }
            ...
            case 'local':
                ...
                if (parts.length > 1 && parts[1].toLowerCase() === 'const') {
                    ...
                    // 检查parts[1]是否以const开头, 如果是则报语法错误
                    if (parts[1].toLowerCase().startsWith('const')) {
                        console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 局部变量声明格式应为 "local [const] 变量名:类型 = 值"`);
                        return;
                    }
                    Interpreter.executeLocal(parts.slice(1).join(' '));
                }
    ...
    static executeGlobal(params) {
        ...
        if (!Interpreter.isValidIdentifier(varName)) {
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 命名错误: 变量名 '${varName}' 不符合命名规则`);
            return;
        }
        ...
        try {
            ...
            const exception = error;
            debugLog(1, `${exception.message} 在第 ${currentLinePointer + 1} 行`);
            return;
        }
    }
    // 执行局部变量声明
    static executeLocal(params) {
        ...
        switch (block.type) {
            ...
            default:
                targetEndTag = '';
                console.error(`switch 分支 运行至 default`);
                break;
        }
        ...
        for (let i = CONTROL_FLOW_STACK.length - 1; i >= 0; i--) {
            ...
            if (block.type === 'for' && block.varName === varName) {
                console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 引用错误: 循环变量 '${varName}' 作用域内禁止声明同名变量`);
                return;
            }
        }
        try {
    ...
    static executeCall(params) {
        ...
        if (matchWithResult) {
            ...
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 函数调用格式应为 "call 函数名(参数1, 参数2, ...) -> 结果变量" 或 "call 函数名(参数1, 参数2, ...)" 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        ...
        if (resultVar === undefined && funcInfo.returnType !== DataType.UNDEFINED) {
            console.error(`错误: 函数 ${funcName} 有返回值, 但未指定结果变量 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        if (resultVar !== undefined && funcInfo.returnType === DataType.UNDEFINED) {
            console.error(`错误: 函数 ${funcName} 无返回值, 但指定了结果变量 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 解析参数
        const args = [];
        if (argsStr.trim()) {
            ...
            console.error(`错误: 函数 ${funcName} 需要 ${funcInfo.params.length} 个参数, 但未提供 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 保存调用所在行号
    ...
    // 执行数组声明
    static executeArrayDeclaration(params, isGlobal, isConst) {
        ...
        if (!arrayMatch) {
            debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 数组声明格式应为 "array arrName[arrLength]:type = [...]" 或 "array arrName[arrLength]:type = arrfill" 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        const arrayName = arrayMatch[1];
        // 检查数组名是否符合C语言命名规则
        if (!Interpreter.isValidIdentifier(arrayName)) {
            debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 命名错误: 数组名 '${arrayName}' 不符合命名规则`);
            return;
        }
        ...
        try {
            ...
            debugLog(1, `错误: 无法解析数组长度表达式 '${lengthExpr}' 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 获取元素类型
        ...
        if (elementType === DataType.UNDEFINED) {
            debugLog(1, `错误: 不支持的数组元素类型 '${elementTypeStr}' 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 检查元素类型是否为不允许的类型
        if (elementType === DataType.ARRAY) {
            debugLog(1, `错误: 不允许声明数组的数组 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 初始化数组元素
        const arrayElements = [];
        ...
        if (initValue === 'arrfill') {
            ...
            switch (elementType) {
                case DataType.NUMBER:
                    debugLog(1, `警告: number类型数组统一填充为0.0, 建议明确声明为int或float类型 在第 ${currentLinePointer + 1} 行`);
                    fillValue = 0.0;
                    break;
                case DataType.INT:
                    fillValue = 0;
            ...
            if (elementValues.length !== arrayLength) {
                debugLog(1, `错误: 数组初始化元素数量(${elementValues.length})与声明长度(${arrayLength})不匹配 在第 ${currentLinePointer + 1} 行`);
                return;
            }
            // 解析每个元素
            for (let i = 0; i < elementValues.length; i++) {
                try {
                    ...
                    debugLog(1, `错误: 无法解析数组元素[${i}]的值 '${elementValues[i]}' 在第 ${currentLinePointer + 1} 行`);
                    return;
                }
            }
        }
        else {
            debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 数组初始化应使用 '[...]' 或 'arrfill' 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 创建数组变量
        const arrayVariable = {
        ...
        if (isGlobal) {
            ...
            if (!currentFunc) {
                debugLog(1, `错误: 不能在函数外部声明局部数组 '${arrayName}' 在第 ${currentLinePointer + 1} 行`);
                return;
            }
            ...
            for (let i = CONTROL_FLOW_STACK.length - 1; i >= 0; i--) {
                ...
                if (block.type === 'for' && block.varName === arrayName) {
                    debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 引用错误: 循环变量 '${arrayName}' 作用域内禁止声明同名变量`);
                    return;
                }
            }
    ...
    static splitArrayElements(elementsStr) {
        const elements = [];
        let currentElement = '';
        let inString = false;
        let stringDelimiter = '';
    ...
    static executeReturn(params) {
        ...
        // 检查是否有返回值
        if (returnValueStr === '') {
            console.error(`错误: return语句后必须跟一个变量 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 获取当前函数名
        // 逆向查找控制流栈中最近的函数调用
        for (let i = CONTROL_FLOW_STACK.length - 1; i >= 0; i--) {
            ...
            if (block.type === 'function') {
                ...
                if (funcName === null) {
                    console.error(`当前返回语句所在行不在函数内`);
                    return;
                }
                else if (funcName !== block.funcName) {
                    console.error(`当前返回语句所在行不在控制流栈顶函数中`);
                    return;
                }
                ...
                if (returnValueStr) {
                    ...
                    console.error(`函数运行时返回值与流程控制栈中的返回值名称不同`);
                    return;
                }
    ...
    static executePrint(params) {
        try {
            ...
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 错误: print 无法计算表达式 '${params}' 在第 ${currentLinePointer + 1} 行`);
            // 如果表达式计算失败, 则返回
            return;
        }
    }
    // 执行if语句
    static executeIf(params) {
        // 检查条件表达式是否用括号括起
        const trimmedParams = params.trim();
        if (!trimmedParams.startsWith('(') || !trimmedParams.endsWith(')')) {
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 条件表达式必须用括号括起 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 提取括号内的表达式
        ...
        try {
            ...
            if (typeof condition !== 'boolean') {
                console.error(`类型错误: 条件表达式必须返回布尔值, 但实际返回了 ${typeof condition} 类型 在第 ${currentLinePointer + 1} 行`);
                return;
            }
            ...
            debugLog(1, `错误: 无效的条件表达式: ${conditionExpr} 在第 ${currentLinePointer + 1} 行`);
    ...
    // 执行while语句
    static executeWhile(params) {
        // 检查条件表达式是否用括号括起
        const trimmedParams = params.trim();
        if (!trimmedParams.startsWith('(') || !trimmedParams.endsWith(')')) {
            debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 条件表达式必须用括号括起 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 提取括号内的表达式
        const conditionExpr = trimmedParams.substring(1, trimmedParams.length - 1);
        try {
            const brokenexists = CONTROL_FLOW_BROKEN_BLOCK_STACK.some(item => item.type === 'while' && item.start === currentLinePointer);
            ...
            // 检查条件表达式的返回值是否为布尔类型
            if (typeof condition !== 'boolean') {
                debugLog(1, `类型错误: 条件表达式必须返回布尔值, 但实际返回了 ${typeof condition} 类型 在第 ${currentLinePointer + 1} 行`);
                return;
            }
            debugLog(2, `当前控制流: `, CONTROL_FLOW_STACK);
            // 先将while循环信息压栈, 防止首次循环条件不满足
            // 检查CONTROL_FLOW_STACK中是否已存在相同while循环信息
            const exists = CONTROL_FLOW_STACK.some(item => item.type === 'while' && item.start === currentLinePointer);
            if (!exists) {
                CONTROL_FLOW_STACK.push({
                    type: 'while',
            ...
            console.error(`错误: 无效的条件表达式: ${conditionExpr} 在第 ${currentLinePointer + 1} 行`);
        }
        debugLog(2, `当前循环结束后控制流: `, CONTROL_FLOW_STACK);
    }
    // 执行endwhl语句
    static executeEndWhile() {
        debugLog(2, `当前控制流: `, CONTROL_FLOW_STACK);
        // 返回到while条件
        let i = currentLinePointer - 1;
        let nestedLevel = 1;
    ...
    static executeFor(params) {
        ...
        if (!match) {
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: for循环格式应为 "for (local 变量名:类型 = 初始值; 条件; 更新表达式)" 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        const varName = match[1];
        const typeStr = match[2];
        ...
        try {
            ...
            const brokenexists = CONTROL_FLOW_BROKEN_BLOCK_STACK.some(item => item.type === 'for' &&
            ...
            if (typeof result !== 'boolean') {
                console.error(`类型错误: 条件表达式必须返回布尔值, 但实际返回了 ${typeof result} 类型 在第 ${currentLinePointer + 1} 行`);
                return;
            }
            debugLog(2, `当前控制流: `, CONTROL_FLOW_STACK);
            // 先将for循环信息压栈, 防止首次循环条件不满足
            // 检查CONTROL_FLOW_STACK中是否已存在相同的for循环信息
            const exists = CONTROL_FLOW_STACK.some(item => item.type === 'for' &&
                item.start === currentLinePointer &&
                item.updateExpr === updateExpr &&
                item.varName === varName);
            if (!exists) {
            ...
            // console.error(`错误: for循环初始化失败 在第 ${currentLinePointer + 1} 行`);
            throw { type: ExceptionType.LOOP_INIT_ERROR, message: `for循环初始化失败 在第 ${currentLinePointer + 1} 行` };
        }
        debugLog(2, `当前循环结束后控制流: `, CONTROL_FLOW_STACK);
    }
    // 执行endfor语句
    static executeEndFor() {
        debugLog(2, `当前控制流: `, CONTROL_FLOW_STACK);
        // 执行更新表达式
        if (CONTROL_FLOW_STACK.length > 0 && CONTROL_FLOW_STACK[CONTROL_FLOW_STACK.length - 1].type === 'for') {
            ...
            try {
                ...
                // console.error(`错误: for循环更新表达式执行失败 在第 ${currentLinePointer + 1} 行`);
                // 跳过循环
                throw { type: ExceptionType.LOOP_UPDATE_ERROR, message: `for循环更新表达式执行失败 在第 ${currentLinePointer + 1} 行` };
            }
        }
    }
    // 执行break语句
    static executeBreak() {
        // 1. 检查是否在合法的控制块内
        if (CONTROL_FLOW_STACK.length === 0) {
            console.error(`[ERROR] break语句不在循环或switch内`);
            return;
        }
        // 2. 逆向查找最近的循环 (而非只看栈顶)
        for (let i = CONTROL_FLOW_STACK.length - 1; i >= 0; i--) {
            ...
            if (block.type === 'for' || block.type === 'switch' || block.type === 'while') {
                ...
                switch (block.type) {
                    ...
                    case 'while':
                        ...
                        break;
                    default:
                        console.error(`[ERROR] 不支持的break上下文`);
                        return;
                }
                ...
                console.error(`[ERROR] 未找到匹配的${targetEndTag}`);
            }
            else {
                CONTROL_FLOW_STACK.pop();
                debugLog(2, `清理后的控制流: `, CONTROL_FLOW_STACK);
    ...
    // 执行continue语句
    static executeContinue() {
        // 1. 检查是否在合法的控制块内
        if (CONTROL_FLOW_STACK.length === 0) {
            console.error(`[ERROR] continue语句不在循环内`);
            return;
        }
        // 2. 逆向查找最近的循环 (而非只看栈顶) 
        for (let i = CONTROL_FLOW_STACK.length - 1; i >= 0; i--) {
            ...
            if (block.type === 'for' || block.type === 'while') {
                ...
                switch (block.type) {
                    ...
                    case 'while':
                        ...
                        break;
                    default:
                        console.error(`[ERROR] 不支持的continue上下文`);
                        return;
                }
                debugLog(2, `跳转目标: ${targetEndTag}`);
                // 4. 跳到对应的结束标记
    ...
    // 执行catch语句
    static executeCatch(params) {
        // 解析catch参数, 格式为 (Exception ErrorName)
        const match = params.match(/^\(\s*Exception\s+([a-zA-Z0-9_]+)\s*\)$/);
        if (!match) {
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: catch语句格式应为 "catch (Exception ErrorName)" 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        ...
        // 情况三: 没有匹配的try块
        console.error(`错误: catch语句没有匹配的try块 在第 ${currentLinePointer + 1} 行`);
        // 跳过catch块
        let nestedLevel = 1;
        let i = currentLinePointer + 1;
        while (i < programLines.length && nestedLevel > 0) {
    ...
    static executeAssert(params) {
        ...
        const trimmedParams = params.trim();
        if (!trimmedParams.startsWith('(') || !trimmedParams.endsWith(')')) {
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 断言表达式必须用括号括起 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 提取括号内的条件
        ...
        try {
            ...
            if (!condition) {
                ...
                if (!messageLine.startsWith('"') || !messageLine.endsWith('"')) {
                    console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: 断言消息必须用双引号括起 在第 ${currentLinePointer + 1} 行`);
                    return;
                }
                // 去掉引号
                ...
                debugLog(1, `断言条件为真: ${conditionExpr} 在第 ${currentLinePointer + 1} 行`);
                // 跳过断言体
                currentLinePointer += 2;
            }
        }
        catch (error) {
            if (error.type === ExceptionType.ASSERTION_ERROR) {
                // 断言失败: 抛出异常交由主循环处理 (可被try-catch捕获; 未捕获时由主循环console.error输出并终止)
                throw error;
            }
            else {
                console.error(`错误: 断言条件无效: ${conditionExpr} 在第 ${currentLinePointer + 1} 行`);
            }
        }
    }
    // 执行switch语句
    static executeSwitch(params) {
        params = params.replace(/^\(|\)$/g, '');
        ...
        try {
            ...
            debugLog(1, `错误: 无效的switch条件表达式: ${params} 在第 ${currentLinePointer + 1} 行`);
        }
    }
    // 执行case语句
    static executeCase(params) {
        debugLog(1, `处理 case 语句`);
        // 检查是否在switch块内
        if (CONTROL_FLOW_STACK.length === 0 || CONTROL_FLOW_STACK[CONTROL_FLOW_STACK.length - 1].type !== 'switch') {
            console.error(`[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: case语句必须在switch块内使用 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        ...
        try {
            ...
            if (switchInfo.type === 'switch' && typeof caseValue !== typeof switchInfo.condition) {
                debugLog(1, `类型错误: case值的类型必须与switch条件类型相同 在第 ${currentLinePointer + 1} 行`);
                // throw {
                //     type: ExceptionType.TYPE_ERROR,
            ...
            debugLog(1, `错误: 无效的case值: ${params} 在第 ${currentLinePointer + 1} 行`);
        }
    }
    // 执行default语句
    static executeDefault() {
        // 检查是否在switch块内
        if (CONTROL_FLOW_STACK.length === 0 || CONTROL_FLOW_STACK[CONTROL_FLOW_STACK.length - 1].type !== 'switch') {
            debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: default语句必须在switch块内使用 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        const switchInfo = CONTROL_FLOW_STACK[CONTROL_FLOW_STACK.length - 1];
    ...
    // 执行endswc语句
    static executeEndSwitch() {
        // 检查是否在switch块内
        if (CONTROL_FLOW_STACK.length === 0 || CONTROL_FLOW_STACK[CONTROL_FLOW_STACK.length - 1].type !== 'switch') {
            debugLog(1, `[ERROR 4] [Line ${currentLinePointer + 1}] 语法错误: endswc语句必须在switch块内使用 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 弹出switch信息
        CONTROL_FLOW_STACK.pop();
    ...
    static executeJump(params) {
        debugLog(1, `参数: ${params}`);
        // 1. 格式校验
        const match = params.match(/^\(([^)]+)\)\s*:\s*([a-zA-Z_]\w*)$/);
        if (!match) {
            console.error(`错误: 必须使用 jump (condition) :标签名 格式 (标签需以字母/下划线开头)  在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 2. 条件解析
        ...
        if (!conditionExpr) {
            console.error(`错误: 条件表达式不能为空 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        let condition;
        try {
            ...
            console.error(`错误: 条件表达式无效: ${conditionExpr} 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        // 3. 条件不满足时直接返回
        if (!condition) {
        ...
        if (TAGS[tagName] === undefined) {
            console.error(`错误: 未定义的标签: ${tagName} 在第 ${currentLinePointer + 1} 行`);
            return;
        }
        currentLinePointer = TAGS[tagName]; // 跳转到标签位置
    }
    // 执行操作指令
    static executeOperation(command) {
    ...
    static checkLoopVarWritable(varName) {
        if (varName === FOR_UPDATE_VAR)
            return; // 循环更新表达式执行中, 豁免只读检查
        for (let i = CONTROL_FLOW_STACK.length - 1; i >= 0; i--) {
            const block = CONTROL_FLOW_STACK[i];
            if (block.type === 'for' && block.varName === varName) {
    ...
    static evaluateExpression(expr) {
        try {
            ...
            // 重新抛出错误, 以便调用者可以处理
            throw new Error(`计算表达式时出错 '${expr}' 在第 ${currentLinePointer + 1} 行: ${e.message}`);
        }
    }
    // 执行清除指令
    static executePurge(params) {
        ...
        if (params.includes('except')) {
            ...
            if (beforeExcept.length !== 1 || beforeExcept[0] !== 'all') {
                console.error(`except关键字必须正确配合all关键字使用 在第 ${currentLinePointer + 1} 行`);
                return;
            }
        ...
        // 清除全局变量
            ...
            let globalVarName = params.slice('global.'.length);
            ...
            if (currentFuncName) {
                ...
                console.error(`函数外不可声明局部变量, 若要清除全局变量请用global关键字`);
            }
        }
    }
    // 执行遇到没有return语句的函数的善后工作
    static executeFunctionEndTag() {
        if (CONTROL_FLOW_STACK.length === 0) {
            console.error(`检测到单独的的函数闭合标记`);
            return;
        }
        else {
            const block = CONTROL_FLOW_STACK[CONTROL_FLOW_STACK.length - 1];
...
class ExpressionEvaluator {
    ...
    // 公共入口方法
    static evaluate(expression, currentLine) {
        // // 检查缓存
    ...
    // 解析数组元素访问
    static parseArrayAccess(arrayName, isGlobal = false) {
    ...
    // 解析数组赋值目标
    static parseArrayAssignmentTarget() {
        const arrayName = this.tokens[this.currentTokenIndex];
        this.currentTokenIndex += 2; // 跳过数组名和 '['
        // 解析索引表达式
        const indexExpr = this.parseExpression();
    ...
    // 解析函数调用
    static parseFunctionCall(funcName) {
    ...
    // 执行函数调用
    static executeFunction(funcName, args) {
        // 支持一些内置函数
        switch (funcName) {
            case 'Math.sin':
                if (args.length !== 1)
                    throw new Error("Math.sin 需要 exactly one argument");
                return Math.sin(args[0]);
            case 'Math.cos':
                if (args.length !== 1)
                    throw new Error("Math.cos 需要 exactly one argument");
                return Math.cos(args[0]);
            case 'Math.tan':
                if (args.length !== 1)
                    throw new Error("Math.tan 需要 exactly one argument");
                return Math.tan(args[0]);
            case 'Math.sqrt':
                if (args.length !== 1)
                    throw new Error("Math.sqrt 需要 exactly one argument");
                return Math.sqrt(args[0]);
            case 'Math.abs':
                if (args.length !== 1)
                    throw new Error("Math.abs 需要 exactly one argument");
                return Math.abs(args[0]);
            case 'Math.pow':
                if (args.length !== 2)
                    throw new Error("Math.pow 需要 exactly two arguments");
                return Math.pow(args[0], args[1]);
            case 'Math.floor':
                if (args.length !== 1)
                    throw new Error("Math.floor 需要 exactly one argument");
                return Math.floor(args[0]);
            case 'Math.ceil':
                if (args.length !== 1)
                    throw new Error("Math.ceil 需要 exactly one argument");
                return Math.ceil(args[0]);
            case 'Math.round':
                if (args.length !== 1)
                    throw new Error("Math.round 需要 exactly one argument");
            ...
            case 'Math.random':
                if (args.length !== 0)
                    throw new Error("Math.random 不需要参数");
                return Math.random();
            case 'len':
                if (args.length !== 1)
                    throw new Error("len 需要 exactly one argument");
            ...
            case 'str':
                if (args.length !== 1)
                    throw new Error("str 需要 exactly one argument");
                return String(args[0]);
            case 'int':
                if (args.length !== 1)
                    throw new Error("int 需要 exactly one argument");
                return parseInt(args[0]);
            case 'float':
                if (args.length !== 1)
                    throw new Error("float 需要 exactly one argument");
    ...
    // 计算二元运算
    static evaluateOperation(operator, left, right) {
    ...
    // 计算一元运算
    static evaluateUnaryOperation(operator, operand) {
        switch (operator) {
            case '+':
                return +operand;
            case '-':
...
function main() {
    ...
    if (typeof process !== 'undefined' && process.argv) {
        ...
        try {
            ...
            console.error(`错误: 无法读取文件 '${filename}': 未知错误类型 - ${error instanceof Error ? error.message : String(error)}`);
            process.exit(1);
        }
    }
    else {
        console.error('此脚本需要在Node.js环境中运行以支持文件读取');
    }
}
// 处理函数返回值赋值逻辑
function handleReturnValueAssignment(funcName, funcInfo, resultVar, oldLinePointer) {
    // 检查函数是否有返回值且调用时有接收变量
