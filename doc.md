...
- `list`：已装模块列表（含孤立检测）；`search <关键词>`：搜索仓库；`remove <包名>`：卸载（含确认）；`repos`：显示生效镜像列表；`init [镜像列表]`：生成镜像配置文件
- 通用参数：`--force`（跳过安全检查/适配检查）、`-y`/`--yes`（所有确认自动答 y）、`--repo <基址[,基址...]>`（临时覆盖仓库镜像列表，逗号分隔多镜像按序回退）

**模块源设置**：模块从哪个仓库拉取由"镜像列表"决定，优先级**`--repo` 命令行 > 配置文件 > 内置默认**：

- **内置默认**（无任何配置时兜底）：官方模块仓库 GitHub 主 + Gitee 镜像双镜像，按序回退（GitHub 失败自动切 Gitee）
- **配置文件**（持久配置）：`{模块目录}/.nsm-mirrors.json`（JSON 对象，`mirrors` 数组字段，上限 4，顺序即回退顺序）。**不必手写 JSON**——用 `nsm -- init` 一键生成：`nsm -- init` 写入内置默认双镜像，`nsm -- init <地址1>,<地址2>` 写入自定义镜像列表；已存在文件覆盖需确认（`--force`/`-y` 跳过）；写入后立即生效
- **`--repo`**（临时覆盖，仅本次运行，不写文件）：支持逗号分隔多基址，单基址则只用它（不回退）
- `nsm -- repos` 随时查看当前生效镜像列表；缺失/坏配置自动回退内置默认
